...
// Scheduled jobs (sweepStaleRidesSandbox, monthlyBillingSandbox,
// monthlyDriverPayoutsSandbox, payoutPendingEarningsSandbox) always run
...
// Mirrors the same flag in functions/index.js. Both servers pin TARGET_DB to
// devDb AND schedule the same jobs at the same times, so LIVE and SANDBOX were
// racing each other against one database — duplicate settlement ledger rows,
// and duplicate stale-ride push notifications (the sweep reads, updates, then
// pushes with no transaction, so two concurrent runs both notify).
// Paused while the driver payout rail is redesigned (Global Payouts turned out
// to be unavailable to Canadian platforms). Set to false to re-enable — and
// when re-enabling, run exactly ONE server's copy of each job, not both.
const SCHEDULED_JOBS_PAUSED = true;
...
// Public https base URL of this sandbox function. Stripe's hosted flows (Identity,
// Connect onboarding) reject custom app schemes as return URLs, so both bounce
// through an https endpoint here that 302s back into the app.
...
app.post("/stripe/connect-webhook", async (req, res) => {
  ...
  try {
    if (event.type === "account.updated") {
      ...
      const fields = connectFieldsFromAccount(acct);
      await devDb.collection("users").doc(uid).set(fields, { merge: true });
    ...
    // A driver's bank rejecting their payout. This MUST be handled here rather
    // than on the platform endpoint: it needs `event.account` to know whose
    // payout bounced, and Stripe only populates that on connected-account
    // events. Subscribed on the platform endpoint it matched nobody, so a driver
    // whose payout was returned was never told.
    if (event.type === "payout.failed") {
      await handlePayoutFailed(event, devDb);
    }
    return res.json({ received: true });
  } catch (err) {
...
// Mirrors functions/index.js — keep the two in sync.
 ...
 *  in functions/index.js — change all three together. */
...
app.post("/cert/adult/session", authenticate, async (req, res) => {
  ...
  const returnUrl = appReturn
    ? `${PUBLIC_BASE_URL}/cert/adult/return?app=${encodeURIComponent(appReturn)}`
    : undefined;
  try {
...
app.post("/cert/student/request", authenticate, async (req, res) => {
  ...
  try {
    ...
    const link = `${PUBLIC_BASE_URL}/cert/student/confirm?token=${token}`;
...
// POST /dev/dispatch-report — explain who /requests/dispatch would notify and,
// for everyone it would skip, why. There is no console on a TestFlight build, so
// without this "nobody got the notification" is opaque: you cannot tell a broken
// push token from an unmatched driver from a wrong-database user doc.
// Returns the same set /drivers/available counts, so a mismatch between the two
// is immediately visible. Read-only — sends nothing.
app.post("/dev/dispatch-report", authenticate, async (req, res) => {
  const db = getDb(req);
  try {
    assertDevEnv(db);
    const usersSnap = await db.collection("users").get();
    const now = Date.now();
    const rows = [];
    // Own eligibility ignoring the self-rule: "would MY device be notified when
    // someone else requests a ride?" is the question being debugged, and the
    // always-present "self" reason would otherwise mask the answer.
    let meEligibility = null;
    usersSnap.docs.forEach((doc) => {
      const u = doc.data() || {};
      const isSelf = doc.id === req.uid;
      // Exactly the rule /requests/dispatch applies, so the report can never drift
      // from the real behaviour.
      const eligible = isEligibleRecipient(u, db, now);
      if (isSelf) meEligibility = eligible;
      const skipReason = isSelf
        ? "self (the requester is never notified)"
        : eligible.reason;
      rows.push({
        uid: doc.id,
        ...
        tokenAgeDays: (() => {
          const age = tokenAgeMs(u, now);
          return age === null ? null : Math.floor(age / 86400000);
        })(),
        wouldNotify: skipReason === null,
        skipReason,
      });
    });
    rows.sort((a, b) => Number(b.wouldNotify) - Number(a.wouldNotify));
    return res.json({
      matching: USE_LEGACY_MATCHING ? "proximity" : "broadcast",
      database: "uniliftdev",
      devTokenMaxAgeDays: DEV_TOKEN_MAX_AGE_DAYS,
      totalUsers: rows.length,
      wouldNotify: rows.filter((r) => r.wouldNotify).length,
      me: rows.find((r) => r.uid === req.uid) ?? null,
      // Would this device be pushed when a DIFFERENT account requests a ride?
      // This is the answer to "why don't I get notifications on my test phone".
      meWouldReceive: meEligibility ? meEligibility.ok : false,
      meBlockedBy: meEligibility
        ? meEligibility.reason
        : "you have no users/ doc in uniliftdev — sign in once while in dev mode",
      drivers: rows,
    });
  } catch (err) {
    console.error("/dev/dispatch-report error:", err);
  ...
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      tx.set(rideRef, {
        ...
        // Never empty — this is what /dev/dropoff range-checks against.
        passengerDropoffs: { [r.passengerId]: dropoff instanceof GeoPoint ? dropoff : pickup },
        joinRequests: {},
        status: "planned",
        started: false,
        maxDetourKm: 10,
...
// POST /dev/dropoff { rideId, passengerId?, driverLat, driverLng } — drop a
// passenger on the bot driver's behalf.
//
// There is NO `confirmed` override. This runs the SAME evaluateDropoff() gate as
// POST /rides/dropoff — it used to take `confirmed` straight from the body
// (defaulting to true), which meant the dev harness never exercised the radius
// check and happily paid the driver for a passenger dropped anywhere.
// `driverLat/driverLng` is where you are pretending the bot is; the Dev Ride
// Panel feeds it from LOCATION_PRESETS. The server decides the outcome.
//
// The `started` and `boarded` preconditions are enforced here too, so the harness
// rehearses the real route rather than a laxer one.
  ...
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      if (ev.distanceKm != null) {
        update[`dropoffDistanceKm.${passengerId}`] = Math.round(ev.distanceKm * 100) / 100;
        update[`dropoffDriverLocation.${passengerId}`] = new GeoPoint(ev.driverLat, ev.driverLng);
        // Mirror into the telemetry field so the passenger's live map moves the
        // way a real driver's broadcast would have moved it.
        update.driverLocation = new GeoPoint(ev.driverLat, ev.driverLng);
      }
      if (ev.disputed) {
        // Kept on the ride so an overcharge dispute is answerable, and so the
        // pattern is visible if a driver does it repeatedly.
...
app.post("/dev/finish", authenticate, async (req, res) => {
  ...
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      if (ride.paymentStatus === "completed") return { ok: true, skipped: true };
      const charge = chargeablePassengers(ride);
      // Mirrors /rides/finish exactly — fare charged == fare credited, no cut.
      // If these two drift, the dev harness stops rehearsing production.
      let driverEarningsCents = 0;
      const now = new Date().toISOString();
      for (const pid of charge) {
        // Same guardrails as /rides/finish: the passenger's own pickup -> their
        // own dropoff, clamped to the fare they were quoted. If these two drift,
        // the dev harness stops rehearsing production.
        const distKm = legDistanceKm(ride, pid) ?? 0;
        const rawFare = calculatePassengerChargeCents(distKm, pricing);
        const quoted = (ride.quotedFareCents || {})[pid];
        const { fareCents, clampedBy } = clampFare(rawFare, quoted);
        if (clampedBy) {
          console.warn("[DEV] ride fare clamped", { rideId, pid, rawFare, fareCents, clampedBy });
        }
        const reserveCents = payoutReserveCents(fareCents, pricing);
        const chargedCents = fareCents + reserveCents;
        driverEarningsCents += fareCents;
      ...
      if (driverEarningsCents > 0) {
        const driverRef = db.collection("users").doc(ride.driverId);
        tx.set(driverRef, { pendingEarningsCents: FieldValue.increment(driverEarningsCents) }, { merge: true });
        // Mirrors /rides/finish. Without this the bot's pendingEarningsCents and
        // its transaction list diverge, which makes "was the driver paid?" hard
        // to assert when testing the dropoff-radius gate.
        tx.set(driverRef.collection("transactions").doc(), {
          type: "ride_earning",
          amount: driverEarningsCents,
          status: "completed",
          description: `[DEV] Earnings — ${charge.length} passenger(s) — ${ride.destination || ""}`,
          createdAt: now,
          rideId,
        });
      }
    ...
    return res.json({
      success: true,
      skipped: !!out.skipped,
      chargedPassengers: out.charged || 0,
      driverEarningsCents: out.driverEarningsCents || 0,
    });
  } catch (err) {
...
async function sweepStaleRidesImpl(db) {
  ...
  const processing = await db.collection("rides")
    .where("paymentStatus", "==", "processing").limit(SWEEP_BATCH).get();
  for (const doc of processing.docs) {
    ...
    if (age != null && age < PROC_TTL_MS) continue;
    const claimed = await claimStale(
      db, doc.ref,
      (d) => d.paymentStatus === "processing",
      { paymentStatus: "pending" },
    );
    if (claimed) summary.processingReset += 1;
  }
...
exports.sweepStaleRidesSandbox = onSchedule(
  ...
  // enough for a job that scans four collections and sends a push per hit.
  {
  ...
  async () => {
    if (SCHEDULED_JOBS_PAUSED) return void console.log("sweepStaleRidesSandbox: PAUSED, skipping");
    try {
...
// availableEarningsCents if positive (monthlyDriverPayoutsSandbox queues it on
// the 5th, once those charges have cleared, and the daily sweeper sends it).
exports.monthlyBillingSandbox = onSchedule(
  // Settlement makes one serial Stripe round trip per user with a balance; the
  // 60s default killed it after a few dozen.
  {
    schedule: "0 3 1 * *",
    timeZone: "America/Toronto",
    timeoutSeconds: 540,
  ...
  async () => {
    if (SCHEDULED_JOBS_PAUSED) return void console.log("monthlyBillingSandbox: PAUSED, skipping");
    try {
...
exports.payoutPendingEarningsSandbox = onSchedule(
  { schedule: "0 4 * * *", timeZone: "America/Toronto", timeoutSeconds: 540, memory: "512MiB" },
  async () => {
    if (SCHEDULED_JOBS_PAUSED) return void console.log("payoutPendingEarningsSandbox: PAUSED, skipping");
    try {
...
// yet collected.
exports.monthlyDriverPayoutsSandbox = onSchedule(
  {
  ...
  async () => {
    if (SCHEDULED_JOBS_PAUSED) return void console.log("monthlyDriverPayoutsSandbox: PAUSED, skipping");
    try {
...
// ── Public profile mirror (dev twin) ─────────────────────────────────────────
// See functions/index.js for why this exists. A v2 Firestore trigger binds to one
// database, so the uniliftdev copy lives here and the uniliftdefault copy lives
// in the LIVE codebase.
exports.mirrorPublicProfileSandbox = onDocumentWritten(
  { document: "users/{uid}", database: "uniliftdev", region: "us-central1" },
  async (event) => {
    const uid = event.params.uid;
    const after = event.data && event.data.after;
    if (!after || !after.exists) {
      try {
        await publicProfileRef(devDb, uid).delete();
      } catch (err) {
        console.warn("mirrorPublicProfileSandbox delete failed for", uid, err.message);
      }
      return;
    }
    await writePublicProfile(devDb, uid, after.data());
  },
);

// ── Auth blocking functions: deliberately NOT here ──────────────────────────
//
// The LIVE codebase exports `beforeUserCreated` / `beforeUserSignedIn` from
// functions/identity.js. They are absent here on purpose, and this is not
// drift to be "fixed".
//
// A Firebase project has exactly ONE beforeCreate URI, and both codebases
// deploy to the same project (unilift-6e756). Exporting them from here as well
// would make the `live` and `sandbox` deploys fight over the same registration,
// with whichever ran last silently winning.
//
// It costs nothing in coverage: Firebase Auth is a single pool per project, so
// the LIVE blocking function already gates every account created against this
// project, sandbox signups included. `scripts/check-server-drift.sh` only reads
// `app.<verb>(` route declarations and does not compare exports, so it will not
// flag this either way.
